...
public class Gendreau06Test {
    ...
    public void overtimeScenario() {
        final Gendreau06Scenario scenario =
                create(1, minutes(6), new AddParcelEvent(new ParcelDTO(
                        new Point(2, 1), new Point(4, 1), new TimeWindow(0,
                                minutes(12)), new TimeWindow(5, minutes(12)),
                        0, 0, 0, 0)));
        final StatisticsDTO dto = runProblem(scenario, useGui);
    ...
    static AddParcelEvent parcelEvent(double x1, double y1, double x2,
            double y2, long tw1b, long tw1e, long tw2b, long tw2e) {
        return new AddParcelEvent(new ParcelDTO(new Point(x1, y1), new Point(
                x2, y2), new TimeWindow(tw1b, tw1e),
    ...
    static Gendreau06Scenario create(int numVehicles, long endTime,
        ...
        final Set<Enum<?>> eventTypes =
                new HashSet<Enum<?>>(asList(PDPScenarioEvent.values()));
    ...
    static class ClosestParcelStrategy implements VehicleStrategy {
        ...
        public void init(DefaultVehicle v, RoadModel rm, PDPModel pm) {
            checkState(
                vehicle == null && roadModel == null && pdpModel == null,
                "init can be called only once!");
            vehicle = v;
            roadModel = rm;
            pdpModel = pm;

            final Set<DefaultDepot> set =
        ...
        public void execute(TimeLapse time) {
            while (time.hasTimeLeft()) {
                final Set<Parcel> parcels =
                        newHashSet(pdpModel.getParcels(ParcelState.AVAILABLE));
                ...
                for (final Parcel p : parcels) {
                    final Point pos =
                            pdpModel.containerContains(vehicle, p) ? p
                                    .getDestination() : roadModel
                                    .getPosition(p);
                    final double d =
                            Point.distance(roadModel.getPosition(vehicle), pos);
                    if (d < dist) {
                        dist = d;
                        closest = p;
                    }
