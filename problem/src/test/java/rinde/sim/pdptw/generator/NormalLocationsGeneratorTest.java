...
import rinde.sim.pdptw.generator.NormalLocationsGenerator;
...
public class NormalLocationsGeneratorTest {
	...
	public void test() {
		...
		final String locationListFileName = new StringBuilder(WORK_DIR).append("locs-").append(fileName)
				.append(".points").toString();

		final String histogramFileName = new StringBuilder(WORK_DIR).append("hist-").append(fileName).append(".data")
				.toString();
	...
	static void writeGeneratorHistogram(NormalLocationsGenerator lg, File f) {
		...
		for (int i = 0; i < histogram.length; i++) {
			sb.append(histogram[i][0]).append(" ").append(histogram[i][1]).append("\n");
		}
		try {
