...
public class DynamicPDPTWProblem {
     ...
     */
    public DynamicPDPTWProblem(final DynamicPDPTWScenario scen,
            long randomSeed, Model<?>... models) {
        simulator =
                new Simulator(new MersenneTwister(randomSeed),
                        scen.getTickSize());
        ...
        final TimedEventHandler handler = new TimedEventHandler() {
            ...
            public boolean handleTimedEvent(TimedEvent event) {
                if (eventCreatorMap.containsKey(event.getClass())) {
                    return ((Creator<TimedEvent>) eventCreatorMap.get(event
                            .getClass())).create(simulator, event);
                } else if (DEFAULT_EVENT_CREATOR_MAP.containsKey(event
                        .getClass())) {
                    return ((Creator<TimedEvent>) DEFAULT_EVENT_CREATOR_MAP
                            .get(event.getClass())).create(simulator, event);
        ...
        final int ticks =
                scen.getTimeWindow().end == Long.MAX_VALUE ? -1 : (int) (scen
                        .getTimeWindow().end - scen.getTimeWindow().begin);
    ...
    public <T extends TimedEvent> void addCreator(Class<T> eventType,
            Creator<T> creator) {
        checkArgument(
            eventType == AddVehicleEvent.class
    ...
    public static abstract class StopCondition extends
         ...
         * dispatched.
         */
        public static final StopCondition VEHICLES_BACK_AT_DEPOT =
                new StopCondition() {
                    @Override
                    public boolean isSatisfiedBy(SimulationInfo context) {
    ...
    public static class DefaultUICreator implements UICreator {
        ...
        protected List<Renderer> createRenderers() {
            return newArrayList(planeRoadModelRenderer(), roadUserRenderer(),
                pdpModelRenderer());
        }
