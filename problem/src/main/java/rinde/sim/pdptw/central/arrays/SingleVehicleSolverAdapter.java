...
public class SingleVehicleSolverAdapter implements Solver {
    ...
    public ImmutableList<ImmutableList<ParcelDTO>> solve(GlobalStateObject state) {
        ...
        final ArraysObject ao =
                ArraysSolvers.toSingleVehicleArrays(state, outputTimeUnit);

        final SolutionObject sol =
                solver.solve(ao.travelTime, ao.releaseDates, ao.dueDates,
                    ao.servicePairs, ao.serviceTimes);

        return ImmutableList.of(ArraysSolvers.convertSolutionObject(sol,
            ao.point2dto, ao.locations));
    }
