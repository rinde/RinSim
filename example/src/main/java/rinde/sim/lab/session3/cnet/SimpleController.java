...
public class SimpleController extends ScenarioController {
	...
	@Override
	protected boolean handleTimedEvent(rinde.sim.scenario.TimedEvent event) {
		if (event.getEventType() == ExampleEvent.ADD_TRUCK) {
			...
			final TruckAgent agent = new TruckAgent(truck, -1, 1);
			...
			final PackageAgent agent = new PackageAgent(p, -1, 1);
			getSimulator().register(agent);
			return true;
		}
		return false;
