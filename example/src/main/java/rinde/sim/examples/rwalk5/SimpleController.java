...
public class SimpleController extends ScenarioController {
	...
	protected boolean createUserInterface() {
		...
		View.startGui(getSimulator(), 4, new ObjectRenderer(roadModel, schema, false), new MessagingLayerRenderer(
				roadModel, schema2));
	...
	protected boolean handleAddTruck(Event e) {
		...
		double minSpeed = 30;
		double maxSpeed = 140;
		RandomWalkAgent agent = new RandomWalkAgent(minSpeed + (maxSpeed - minSpeed) * randomizer.nextDouble(), radius,
				0.01 + randomizer.nextDouble() / 2);
		getSimulator().register(agent);
		agent.eventAPI.addListener(statistics, RandomWalkAgent.Type.FINISHED_SERVICE);
		// it is important to inform controller that event was handled to avoid
		// runtime exception
		return true;
	}
