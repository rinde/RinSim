...
public interface State<E, C> {
    ...
    /**
     * Should handle the event. Allows the state to take control and react to
     * the event.
     ...
     *         supported by this state (as defined by the {@link StateMachine})
     *         a {@link RuntimeException} will be thrown by the
     *         {@link StateMachine}.
     */
    E handle(E event, C context);

    /**
     * This method is called at the moment the {@link StateMachine} 'enters'
     * this state (i.e. during a state transition). It is called just before the
