...
public class GraphRoadModel extends AbstractRoadModel<Loc> {
    ...
    protected PathProgress doFollowPath(MovingRoadUser object,
        ...
        while (time.hasTimeLeft() && path.size() > 0) {
            ...
            if (travelDistance >= connLength) {
                ...
                final Point from = isMidPoint(tempLoc) ? tempLoc.conn.from
                        : tempLoc;
                final Point peekTo = isMidPoint(path.peek()) ? ((Loc) path
    ...
    protected void checkIsValidMove(Loc objLoc, Point nextHop) {
        ...
        else if (!objLoc.isEdgePoint() && !nextHop.equals(objLoc)
    ...
    protected Loc checkLocation(Loc l) {
        checkArgument(l.isEdgePoint() || graph.containsNode(l), "Location points to non-existing vertex: "
                + l + ".");
        checkArgument(!l.isEdgePoint()
    ...
    protected Connection<?> getConnection(Point from, Point to) {
        final boolean fromIsMid = isMidPoint(from);
        final boolean toIsMid = isMidPoint(to);
    ...
    final protected static class Loc extends Point {
        ...
        private Loc(double pX, double pY,
                Connection<? extends ConnectionData> pConn, double pRoadLength,
                double pRelativePos) {
            super(pX, pY);
