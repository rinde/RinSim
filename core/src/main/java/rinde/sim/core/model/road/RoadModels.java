...
public class RoadModels {
	...
	public static <T extends RoadUser> List<T> findClosestObjects(Point pos, RoadModel rm, Collection<T> objects, int n) {
		return Graphs.findClosestObjects(pos, objects, new RoadModels.RoadUserToPositionFunction<T>(rm), n);
	}
	...
	public static <T extends RoadUser> Collection<T> findObjectsWithinRadius(final Point position,
		...
		return RoadModels.findObjectsWithinRadius(position, model, radius, model.getObjectsOfType(type));
	}
	...
	public static <T extends RoadUser> Collection<T> findObjectsWithinRadius(final Point position,
		...
		return Collections2.filter(objects, new RoadModels.DistancePredicate(position, model, radius));
	}
